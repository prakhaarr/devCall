...
function DashboardPage() {
  ...
  const interviews = useQuery(api.interviews.getAllInterviews);
  const updateStatus = useMutation(api.interviews.updateInterviewStatus);
  const removeInterview = useMutation(api.interviews.removeInterview);

  const [selectedInterview, setSelectedInterview] =
    useState<Id<"interviews"> | null>(null);
  ...
  const handleCancelInterview = async () => {
    ...
    try {
      await removeInterview({ id: selectedInterview });
      toast.success("Interview canceled successfully");
      setSelectedInterview(null); // Close the dialog
    } catch (error) {
